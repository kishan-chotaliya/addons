...
class SaleOrder(models.Model):
    ...
    final_sale_price = fields.Float(compute='_compute_final_sale_price', store=True)
    ref = fields.Many2one('sale.order')
    quote_count = fields.Integer(compute='_compute_quote_count', store=True)
...
class GroupLine(models.Model):
    ...
    total_sale = fields.Float(compute='_compute_sale_total', string='Sale Price Total', store=True)
    total_cost = fields.Float(compute='_compute_cost_total', string='Cost Price Total', store=True)
    total_margin = fields.Float(compute='_compute_margin_total', string='Margin Total', store=True)
...
class GroupOrderLine(models.Model):
    ...
    product_id = fields.Many2many('product.product', compute='_compute_product_ids', string='Product Name', store=True)
    # qty = fields.Float(string="Quantity", required=True, default=1.0)
    desc = fields.Char(string='Description', required=True)
    cost_price = fields.Float(compute='_compute_cost_total', store=True)
    # sale_price_per_unit = fields.Float(related='product_id.list_price', string="Recommended Sale Price")
    total_sale_price = fields.Float(compute='_compute_sale_total', string="Total Sale Price", store=True)
    difference = fields.Float(compute='_compute_margin_total', string="Margin", store=True)
...
class GroupOrderLineLine(models.Model):
    ...
    cost_price = fields.Float(compute='_compute_product_cost', store=True)
    # sale_price_per_unit = fields.Float(related='product_id.list_price', string="Recommended Sale Price")
    total_sale_price = fields.Float(string="Total Sale Price")
    difference = fields.Float(compute='_compute_diff', string="Margin", store=True)
